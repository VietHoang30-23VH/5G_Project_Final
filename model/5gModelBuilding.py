...
""":# **Xem nội dung của dataset và chọn lọc 25 trường thông tin**

Xem các cột có trong dataset
"""
...
print("- Danh sách các cột:", df_full.columns.tolist())
cols = df_full.columns.tolist()
for i in range(0, len(cols), 15):
    print("  " + " | ".join(cols[i:i+15]))

"""Chọn các trường thông tin và tạo 1 dataset mới từ các trường đó"""
...
print(f"-- Result : Dataset mới đã được lưu tại {output_path}")

"""# **Xem thông tin của dataset mới vừa tạo**"""
...
for i in range(0, len(cols), 15):
    print("  " + " | ".join(cols[i:i+15]))
print(f"- Số lượng benign traffic: {num_benign}")
print(f"- Số lượng malicious traffic: {num_malicious}")
print(f"- Độ chênh lệnh giữa benign và malicious: {num_dif_benign_malicious}")
...
"""# **Split data và xử lí dữ liệu bị thiếu**
- Xóa dữ liệu bị thiếu
- Xóa các cột IP

=> Giữ lại khiến mô hình dễ phụ thuộc thay vì phân tích lưu lượng.
- Thực hiện Pearson Correlation: loại bỏ các đặc trưng dư thừa, có nghĩa là các đặc trưng tương quan cao, mang thông tin giống nhau. Chỉ giữ lại một đặc trưng duy nhất.
- Thực hiện phân tích ANOVA F-scores: đưa ra 25 trường có ảnh hưởng cao nhất đến kết quả.

**Chia dữ liệu train/set**
"""
...
def check_missing_values(df):
    ...
    if missing_df.empty:
        ...
        print(missing_df)
        # Vẽ biểu đồ trực quan hóa dữ liệu bị thiếu
        plt.figure(figsize=(10, 5))
        missing_df['Missing Values'].plot(kind='bar', color='red')
        plt.title('Số lượng giá trị bị thiếu trong mỗi cột')
        plt.xlabel('Cột')
        plt.ylabel('Số lượng')
        plt.xticks(rotation=45)
        plt.show()

print("--- Kiểm tra ở trainning set ---")
...
"""Tiến hành đánh giá model và tải xuống model"""
...
print(f"- ROC AUC      : {roc_auc:.4f}" if roc_auc else "- ROC AUC      : Không tính được với dữ liệu này")
print(f"- Execution Time: {execution_time:.2f} giây")
...
# Tạo DataFrame cho confusion matrix để dễ hiển thị
...
plt.show()
print('='*200)
# Tạo normalized confusion matrix (tỷ lệ phần trăm)
cm_normalized = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis]
cm_normalized_df = pd.DataFrame(cm_normalized, index=labels, columns=labels)

# Vẽ heatmap của normalized confusion matrix
...
# Lấy các cột phù hợp với tập huấn luyện
...
# Chỉ chọn các cột cần thiết cho mô hình
csv_data = csv_data[X_train.columns]
